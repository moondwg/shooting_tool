...
enum AppState { STATE_MENU, STATE_SHOOTING_TOOL };
...
void drawMenu() {
  ...
  canvas.drawString("== Ballistic Tool ==", canvas.width() / 2, 20);
  canvas.drawString(menuIndex == 0 ? "> Launch Shooting Tool <" : "  Launch Shooting Tool  ", canvas.width() / 2, canvas.height() / 2);
...
void loop() {
  ...
  if (appState == STATE_MENU) {
    if (M5Cardputer.Keyboard.isKeyPressed(0x0A)) {  // Enter key
      if (menuIndex == 0) {
    ...
    if (M5Cardputer.Keyboard.isKeyPressed(KEY_FN)) {  // Simulated UP
      menuIndex = (menuIndex - 1 + MENU_COUNT) % MENU_COUNT;
      drawMenu();
      delay(150);
    }

    if (M5Cardputer.Keyboard.isKeyPressed(KEY_CTRL)) {  // Simulated DOWN
      menuIndex = (menuIndex + 1) % MENU_COUNT;
      drawMenu();
      delay(150);
    }
  } else if (appState == STATE_SHOOTING_TOOL) {
